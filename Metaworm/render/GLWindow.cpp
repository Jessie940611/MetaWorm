...
GLWindow::GLWindow():mCamera(new camera()),mIsDrag(false),mPrevX(0),mPrevY(0),mDisplayTimeout(1.0/30.0)
{
...
void GLWindow::InitWindow(int _w,int _h,const char* _name)
{
...
void GLWindow::DisplayEvent()
{
	current()->Display();
}

void GLWindow::KeyboardEvent(unsigned char key,int x,int y)
{
...
void GLWindow::MotionEvent(int x, int y)
{
	current()->Motion(x,y);
}

void GLWindow::ReshapeEvent(int w, int h)
{
	current()->Reshape(w,h);
}

void GLWindow::TimerEvent(int value)
{
	current()->Timer(value);
}