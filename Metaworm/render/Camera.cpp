...
camera::camera() :fovy(60.0), lookAt(Eigen::Vector3d(0, 0.1, 0)), eye(Eigen::Vector3d(0, 1, 3)), up(Eigen::Vector3d(0, 1, 0))
{
...
void camera::Rotate(int x,int y,int prev_x,int prev_y, int w, int h)
{
	// GLint w = glutGet(GLUT_WINDOW_WIDTH);
	// GLint h = glutGet(GLUT_WINDOW_HEIGHT);
