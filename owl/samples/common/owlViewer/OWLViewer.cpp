...
namespace owl {
  namespace viewer {
    ...
    void OWLViewer::resize(const vec2i &newSize)
    {
      ...
      if (rc != cudaSuccess)
      {
        fprintf(stderr, "cuda cannot share graphics resources. error (%s: line %d): %s\n", __FILE__, __LINE__, cudaGetErrorString(rc));
        shareGraphicResource = false;
        if (cuDisplayTexture)
        {
          cudaGraphicsUnregisterResource(cuDisplayTexture);
          cuDisplayTexture = 0;
    ...
    void OWLViewer::draw()
    {
      ...
      glEnable(GL_TEXTURE_2D);
      glBindTexture(GL_TEXTURE_2D, fbTexture);
      //glBindTexture(GL_TEXTURE_2D, cuDisplayTexture);
