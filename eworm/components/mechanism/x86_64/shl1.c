...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 shl1 /home/brains/worm_simulation/interact/worm-simulation/eworm/components/mechanism/modfile/shl1.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {int _reset = 0; {
   ...
   Dm = ( minf - m ) / ( tm * 0.4 ) ;
   Dhf = ( hfinf - hf ) / ( thf * 0.08 ) ;
   Dhs = ( hsinf - hs ) / ( ths * 0.3 ) ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) {
 setparames ( _threadargscomma_ v ) ;
 Dm = Dm  / (1. - dt*( ( ( ( - 1.0 ) ) ) / ( tm * 0.4 ) )) ;
 Dhf = Dhf  / (1. - dt*( ( ( ( - 1.0 ) ) ) / ( thf * 0.08 ) )) ;
 Dhs = Dhs  / (1. - dt*( ( ( ( - 1.0 ) ) ) / ( ths * 0.3 ) )) ;
  return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) { {
   setparames ( _threadargscomma_ v ) ;
    m = m + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / ( tm * 0.4 ))))*(- ( ( ( minf ) ) / ( tm * 0.4 ) ) / ( ( ( ( - 1.0 ) ) ) / ( tm * 0.4 ) ) - m) ;
    hf = hf + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / ( thf * 0.08 ))))*(- ( ( ( hfinf ) ) / ( thf * 0.08 ) ) / ( ( ( ( - 1.0 ) ) ) / ( thf * 0.08 ) ) - hf) ;
    hs = hs + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / ( ths * 0.3 ))))*(- ( ( ( hsinf ) ) / ( ths * 0.3 ) ) / ( ( ( ( - 1.0 ) ) ) / ( ths * 0.3 ) ) - hs) ;
   }
  return 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/brains/worm_simulation/interact/worm-simulation/eworm/components/mechanism/modfile/shl1.mod";
static const char* nmodl_file_text = 
