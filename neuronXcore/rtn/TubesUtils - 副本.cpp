...
namespace rtn {
  ...
  */
  ...
  Tubes::SP trySWC_folder_Method1(const std::string& fileName, unsigned voltagesSize = 1998 )
  {
      ...
      //std::ifstream rec_voltagesIn(rec_voltagesFile, std::ios_base::binary);
      ...
      neurons->primColorsVec.resize(0);

      FILE* fp;
      fp = fopen(rec_voltagesFile.c_str(), "rb");
      int rec_len, nrec, nlast, len, nparam;
      fscanf(fp, "%d%d%d%d\n", &nparam, &rec_len, &nrec, &nlast);
      len = rec_len * nrec + nlast;
      float* rec_vals = new float[nparam * len];
      fread(rec_vals, sizeof(float), nparam * len, fp);
      ...
      for (int i = 0; i < len; i++)
      {
          ...
          neurons->primColorsVec.push_back(aa);
      }
      ...
      std::cout << OWL_TERMINAL_GREEN
          << "#neurons: loaded .swc-like folder file with "
          << prettyNumber(neurons->primColorsVec[0].size()) << " rec_voltages "
          << prettyNumber(neurons->primColorsVec.size()) << " nodes "
          << std::endl
      ...
      while (std::getline(indicesin, line))
      {
          ...
          neurons->lineStrips.back().push_back({ positions[cur],radii[cur],colorsID[cur] });
          neurons->lineStrips.back().push_back({ positions[prev],radii[prev],colorsID[prev] });
      }
  ...
  */
  Tubes::SP Tubes::load(const std::string& fileName,
                        float radius, unsigned volSz)
  {
      if (Tubes::SP tubes = trySWC_folder_Method1(fileName, volSz))
      {
                        ...
                        float radius, unsigned volSz)
  {
    ...
    {
      Tubes::SP tubes = load(fileNames[i],radius, volSz);
      if (tubes != nullptr)
      {
        if (result != nullptr)
        {
